...
class Role extends Model implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}roles(name,description)values('$this->name','$this->description')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}roles set name='$this->name',description='$this->description' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,description from {$tx}roles");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,description from {$tx}roles $criteria limit $top,$perpage");
		$data=[];
		while($role=$result->fetch_object()){
			$data[]=$role;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,description from {$tx}roles where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		while($role=$result->fetch_object()){
			...
			$html.="<tr><td>$role->id</td><td>$role->name</td><td>$role->description</td> $action_buttons</tr>";
		}
		$html.="</table>";
	...
	static function html_row_details($id){
		...
		$html.="<tr><th>Description</th><td>$role->description</td></tr>";
